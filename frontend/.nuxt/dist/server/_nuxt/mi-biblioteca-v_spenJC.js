import { defineComponent, mergeModels, useModel, mergeProps, useSSRContext, ref, watch, unref } from "vue";
...
const _sfc_main$1 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props, { emit: __emit }) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      if (show.value) {
        _push(`<div${ssrRenderAttrs(mergeProps({ style: { "position": "fixed", "inset": "0", "display": "grid", "place-items": "center", "background": "rgba(0,0,0,.5)" } }, _attrs))}><div class="card" style="${ssrRenderStyle({ "max-width": "420px" })}"><p>${ssrInterpolate(props.message || "¿Confirmar?")}</p><div style="${ssrRenderStyle({ "display": "flex", "gap": "8px", "justify-content": "flex-end" })}"><button class="button secondary">Cancelar</button><button class="button danger">Eliminar</button></div></div></div>`);
      } else {
        _push(`<!---->`);
      }
    };
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<div${ssrRenderAttrs(mergeProps({
        class: "grid",
        style: { "gap": "16px" }
      }, _attrs))}><div class="card" style="${ssrRenderStyle({ "display": "grid", "gap": "8px" })}"><strong>Buscar &amp; Filtrar</strong><div class="grid" style="${ssrRenderStyle({ "grid-template-columns": "1fr 1fr 1fr 1fr", "gap": "8px" })}"><input class="input"${ssrRenderAttr("value", q.value)} placeholder="Buscar por título"><input class="input"${ssrRenderAttr("value", author.value)} placeholder="Buscar por autor"><select class="select"><option value="rating_desc"${ssrIncludeBooleanAttr(Array.isArray(sort.value) ? ssrLooseContain(sort.value, "rating_desc") : ssrLooseEqual(sort.value, "rating_desc")) ? " selected" : ""}>Rating ↓</option><option value="rating_asc"${ssrIncludeBooleanAttr(Array.isArray(sort.value) ? ssrLooseContain(sort.value, "rating_asc") : ssrLooseEqual(sort.value, "rating_asc")) ? " selected" : ""}>Rating ↑</option></select><label style="${ssrRenderStyle({ "display": "flex", "align-items": "center", "gap": "8px" })}"><input type="checkbox"${ssrIncludeBooleanAttr(Array.isArray(exclude.value) ? ssrLooseContain(exclude.value, null) : exclude.value) ? " checked" : ""}> Excluir sin review </label></div></div>`);
      if (unref(store).loading) {
        ...
        ssrRenderList(unref(store).items, (b) => {
          ...
          if (b.year) {
            _push(`<span>· ${ssrInterpolate(b.year)}</span>`);
          } else {
            _push(`<!---->`);
          }
          _push(`</small></div>`);
...
//# sourceMappingURL=mi-biblioteca-v_spenJC.js.map
